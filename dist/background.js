...
exports.default = exports.noop = exports.getHostname = exports.makeLogger = void 0;
...
var _makeLogger = (0, _utils.makeLogger)('BKG'),
    log = _makeLogger.log,
...
var recordBlocked = function recordBlocked(hostname) {
  ...
  browser.storage.local.set({
    blocked: blocked
  }).then(_utils.noop, onError);
};

var setDomain = function setDomain(val) {
  domain = val;
};
...
document.addEventListener('DOMContentLoaded', function () {
  browser.tabs.query({
    ...
    return setDomain((0, _utils.getHostname)(tabs.map(function (tab) {
      return tab.url;
    })[0]));
  });
});
Promise.all([fetch('./../BLOCKLIST').then(function (res) {
  return res.text();
}), browser.storage.sync.get('userblocklist').then(function (res) {
  return res.userblocklist ? res.userblocklist.split('\n') : [];
  ...
  var handleBeforeRequest = function handleBeforeRequest(request) {
    var hostname = (0, _utils.getHostname)(request.url);

    if (hostname.trim().length > 0 && list.indexOf(hostname) > -1) {
      log("BLOCKED: ".concat(hostname));
...
},{}]},{},["../../.config/yarn/global/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","background.js"], null)
