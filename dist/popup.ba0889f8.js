...
var setDomainHTML = function setDomainHTML(url) {
  domain.innerHTML = (0, _utils.getHostname)(url);
};

var blockedlist = document.getElementById('blockedlist');

var setBlockedlistHTML = function setBlockedlistHTML(url) {
  browser.storage.local.get().then(function (_ref) {
    var _ref$blocked = _ref.blocked,
        blocked = _ref$blocked === void 0 ? {} : _ref$blocked;
    var list = blocked[(0, _utils.getHostname)(url)];
    if (typeof list === 'undefined' || list.length === 0) return;

    var blockedlistHTML = _toConsumableArray(new Set(blocked[(0, _utils.getHostname)(url)])).map(function (hostname, i) {
